...
import {
  ...
  getAxiosMovieCredits,
  getAxiosVideos,
  getAxiosRecommendations,
} from "../../Hooks";
...
const MovieDetails: React.FC = () => {
  ...
  const clickOnFav = async () => {
    if (!context.user){
      ...
      if (!isClicked) {
        ...
        const favouriteMovieIds = await getSelectedFavourite(details);
  ...
  //     if (!isClicked) {
  //       if (context.user) await addFavourite(details, context.user.uid);
  //     } else {
  //       const favouriteMovieIds = await getSelectedFavourite(
  //         details
  //       );
  //       removeFavourite(favouriteMovieIds);
  //     }
  ...
  const myStyledBackground = {
    backgroundImage: `url(${
      IMAGE_URL + IMAGE_SIZE_1920 + details?.backdrop_path
    })`,
  ...
  const deneme = async () => {
    if (params.id) {
      ...
      getAxiosMovieDetails(params.id)
        .then((data) => {
          setDetails(data);
        })
        .catch((error) => {
      ...
      getAxiosMovieCredits(params.id)
        .then((data) => {
          setCredits(data);
        })
        .catch((error) => {
      ...
      getAxiosRecommendations(params.id)
        .then((data) => {
          ...
          } else setNewRecommendations(data);
        })
        .catch((error) => {
  ...
  const deneme2 = async () => {
    if (details && context.user) {
      const isClickedFromFireStore = await queryData2(details, context.user.uid);
      setIsClicked(isClickedFromFireStore)
    }
  };
      ...
      <div className="info-section">
        <div className="yazi-section">
          <div>
            ...
            />
            <Tooltip id="back-tooltip" style={{ zIndex: "99" }} />
            <HomeOutlined
            ...
            />
            <Tooltip id="home-tooltip" style={{ zIndex: "99" }} />
          </div>
          <h1 className="yazi">
            ...
            <div onClick={clickOnFav}>
              <HeartIcon
                ...
                data-tooltip-content={
                  isClicked ? "Favorilerden Kaldır" : "Favorilere Ekle"
                }
                style={{
                  color: isClicked ? "red" : "white",
                  opacity: isClicked ? "" : "0.6",
                  cursor: "pointer",
                  userSelect: "none",
                }}
              />
              <Tooltip id="fav-tooltip" style={{ zIndex: "99" }} />
            </div>
          }
        </div>
        ...
        <div className="img-section">
          <img
            ...
            src={`${
              details?.poster_path
                ? IMAGE_URL + IMAGE_SIZE_500 + details?.poster_path
                : NoPosterImg
            }`}
        ...
        <div className="site-statistic-demo-card">
          <Cards
            baslik={"Beğenilme Oranı"}
            icon={<LikeOutlined />}
            yazi={details?.vote_average && details.vote_average * 10}
            suffix={"%"}
          />
          <Cards
            baslik={"Bütçe"}
            icon={<DollarOutlined />}
            yazi={details?.budget && details.budget}
            suffix={""}
          />
          <Cards
            baslik={"18+"}
            icon={details?.adult ? <WarningOutlined /> : <SafetyOutlined />}
            yazi={details?.adult ? "Evet" : "Hayır"}
            suffix={""}
          />
          {credits?.crew
            ...
            .map((filteredName) => {
              return (
                <Cards
                  key={filteredName.credit_id}
                  baslik={"Yönetmen"}
                  icon={<VideoCameraOutlined />}
                  yazi={filteredName.name}
                  suffix={""}
                />
              );
            })}
              ...
              return (
                <div className="actor-div">
                  <img
                    ...
                    src={
                      e.profile_path
                        ? IMAGE_URL + IMAGE_SIZE_500 + e.profile_path
                        : NoPosterImg
                    }
                  />
        ...
        {newRecommendations.length !== 0 && (
          <>
            <div className="ust-bolum">
              ...
              <Button
                onClick={() => navigate(`/recommendations/${params.id}`)}
